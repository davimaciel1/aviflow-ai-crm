...
const KanbanBoard = () => {
  ...
  const saveEdit = (dealId: string) => {
    setStages(prevStages => prevStages.map(stage => ({
      ...stage,
      deals: stage.deals.map(deal => deal.id === dealId ? {
        ...deal,
  ...
  const handleAddDeal = () => {
    ...
    setStages(prevStages => prevStages.map(stage => stage.id === selectedStageId ? {
      ...stage,
  ...
  return <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        {!isClientView && <Dialog open={isAddingDeal} onOpenChange={setIsAddingDeal}>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input id="title" value={newDealTitle} onChange={e => setNewDealTitle(e.target.value)} placeholder="Nome do projeto" />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={selectedStageId} onValueChange={setSelectedStageId}>
                      ...
                      <SelectContent>
                        {stages.map(stage => <SelectItem key={stage.id} value={stage.id}>
                            {stage.title}
                          </SelectItem>)}
                      </SelectContent>
                    </Select>
                ...
                <div className="space-y-2">
                  ...
                  <Textarea id="description" value={newDealDescription} onChange={e => setNewDealDescription(e.target.value)} placeholder="Descreva o projeto..." rows={3} />
                </div>

                <div className="grid grid-cols-3 gap-4">
                  <div className="space-y-2">
                    <Label htmlFor="value">Valor (R$)</Label>
                    <Input id="value" type="number" value={newDealValue} onChange={e => setNewDealValue(e.target.value)} placeholder="150000" />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={newDealClient} onValueChange={setNewDealClient}>
                      ...
                      <SelectContent>
                        {mockClients.map(client => <SelectItem key={client.id} value={client.id}>
      ...
      {isClientView && stages.length === 0 && <div className="text-center py-12">
      ...
      <DragDropContext onDragEnd={handleDragEnd}>
        <div className="flex space-x-6 overflow-x-auto pb-6">
          {stages.map(stage => <div key={stage.id} className="flex-shrink-0 w-80">
              <div className="bg-slate-50 rounded-lg p-4">
                ...
                <Droppable droppableId={stage.id}>
                  {(provided, snapshot) => <div ref={provided.innerRef} {...provided.droppableProps} className={`space-y-3 min-h-[200px] ${snapshot.isDraggingOver ? 'bg-blue-50 rounded-lg p-2' : ''}`}>
                      {stage.deals.map((deal, index) => <Draggable key={deal.id} draggableId={deal.id} index={index}>
                          {(provided, snapshot) => <div ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps} className={`${snapshot.isDragging ? 'rotate-2 shadow-lg' : ''}`}>
                              <Card className="cursor-grab hover:shadow-md transition-shadow bg-white">
                                <CardContent className="p-4">
                                  <div className="space-y-3">
                                    {/* Header with expand/collapse button */}
                                    <div className="flex items-start justify-between">
                                      {editingCard === deal.id ? <div className="flex-1 space-y-2">
                                ...
                                description: e.target.value
                                          ...
                                          <div className="flex-1">
                                            ...
                                            {deal.description && <p className="text-xs text-slate-600 line-clamp-2 mb-2">
                                                {deal.description}
                                              </p>}
                                          </div>
                                          <div className="flex items-center space-x-1">
                                            {!isClientView && <Button size="sm" variant="ghost" onClick={() => startEditing(deal)} className="h-6 w-6 p-0">
                                                <Edit className="w-3 h-3" />
                                              </Button>}
                                            <Button size="sm" variant="ghost" onClick={() => toggleCardExpand(deal.id)} className="h-6 w-6 p-0">
                                              {expandedCards.has(deal.id) ? <ChevronUp className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />}
                                            </Button>
                                          </div>
                                        </>}
                                    </div>
                                    ...
                                    {deal.client && <div className="flex items-center justify-between">
                                    ...
                                    {expandedCards.has(deal.id) && <div className="space-y-3 pt-2 border-t border-slate-100">
                                        {/* Confidential Information */}
                                        {deal.confidential && <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
                                        ...
                                        {/* Notes */}
                                        {deal.notes && deal.notes.length > 0 && <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                                            <div className="flex items-start space-x-2">
                                              ...
                                              <div className="flex-1">
                                                ...
                                                <div className="space-y-1">
                                                  {deal.notes.map((note, index) => <p key={index} className="text-xs text-blue-700">
                                                      • {note}
                                                    </p>)}
                                                </div>
                                              </div>
                                        ...
                                        {/* Additional details */}
                                        <div className="space-y-2">
                                          {deal.contact && <div className="flex items-center space-x-2">
                                          ...
                                          {deal.client && <div className="space-y-1">
                                              <div className="flex items-center space-x-2">
                                                <Mail className="w-3 h-3 text-slate-400" />
                                                <span className="text-xs text-slate-600">{deal.client.email}</span>
                                              </div>
                                              {deal.client.phone && <div className="flex items-center space-x-2">
